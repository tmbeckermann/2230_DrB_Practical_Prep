...
const state = {
  ...
  visualIndex: 0,
  visualKey: '',
  checked: loadJson('ul_checked', {}),
  drill: loadJson('ul_drill', {}),
  drillAnswers: loadJson('ul_drill_answers', {}),
  oiaSelections: loadJson('ul_oia_selections', {}),
...
const PRACTICE_FOCUS_STATUS = {
  all: 'Showing every activity in the upper-limb practice library.',
  bone: 'Showing bone and marking identification activities only.',
  muscle: 'Showing muscle labeling, image, and arm-model identification activities only.',
...
function applyPracticeFocus(requestedFocus = 'all') {
  ...
  root.querySelectorAll('.activity-library-group').forEach((group) => {
...
function setChecked(kind, id, value) {
  ...
  saveJson('ul_checked', state.checked);
  renderDashboard();
}
...
function muscleReferenceProvenance(row) {
  ...
  return {
    ...
    sourceDescription: referenceOnly
      ? 'PAL atlas image for anatomy review. It is not a course lab-model photo and is not part of the tested arm-model identification list.'
      : source.description || 'PAL atlas art used as a substitute for model-ID practice. This is not a course lab-model photo.',
  ...
  return `<div class="drill-image-grid">
    ${items.map((item) => `<figure>
      <span class="drill-image-frame">
        ...
        ${item.hideSideCues ? '<span class="side-cue-mask side-cue-mask-top" aria-hidden="true"></span><span class="side-cue-mask side-cue-mask-left" aria-hidden="true"></span><span class="side-cue-mask side-cue-mask-right" aria-hidden="true"></span>' : ''}
      </span>
...
function isFullBoneSideVisual(visual) {
  ...
  return ['scapula', 'clavicle', 'humerus', 'radius', 'ulna', 'hand'].some((term) => title.includes(term));
}
...
function drillDecks() {
  ...
  const leftRightCards = leftRightSource
    .flatMap((row) => {
      ...
      const hideSideCues = row.hideSideCues === true || !usingCroppedLeftRight;
      const statusGroups = [{
      ...
      return [
        {
          ...
          images: [imageCardItem(row, { sourceTitle: '', hideSideCues })],
          side: sourceSide,
          view,
          textResponse: true,
        ...
        },
        {
          ...
          images: [imageCardItem(row, { mirrored: true, sourceTitle: '', hideSideCues })],
          side: mirroredSide,
          view,
          textResponse: true,
          responsePlaceholder: 'Type left, right, or unsure.',
...
function hugRegionNames() {
  const preferred = ['Scapula', 'Clavicle', 'Humerus', 'Radius', 'Ulna', 'Hand'];
...
function resumeHugRegion() {
  ...
  }) || regions.find((region) => hugRegionProgress(region).correct < hugRegionProgress(region).total) || regions[0] || 'Scapula';
}
...
function renderRegionMap() {
  const regions = [
    { label: 'Scapula', icon: 'SC', note: 'borders, fossae, and processes' },
    { label: 'Clavicle', icon: 'CL', note: 'ends, borders, and tubercles' },
    { label: 'Humerus', icon: 'HM', note: 'proximal and distal landmarks' },
    { label: 'Radius', icon: 'RA', note: 'lateral forearm landmarks' },
    { label: 'Ulna', icon: 'UL', note: 'medial forearm landmarks' },
    { label: 'Hand', icon: 'HD', note: 'carpals, metacarpals, and phalanges' }
